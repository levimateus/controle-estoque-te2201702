...
class ProdutosTest extends TestCase
{
		...
		public function test_usuarios_logados_podem_exibir_produtos_de_um_fornecedor(){
			...
			$fornecedor = factory('App\Fornecedor')->create();
			$produtos = factory(\App\Produto::class, 5)->make(['fornecedor_id' => $fornecedor->id]);
			...
			$response = $this->withExceptionHandling()
						     ->actingAs($usuario)
						     ->call('GET', '/fornecedor/'.$fornecedor->id.'/produtos');
			...
			foreach ($produtos as $produto) {
	            ...
	            $response->assertSee($produto->fornecedor_id);
        	}	
